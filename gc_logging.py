...
def thread_function():
    ...
    list_id = ["48", "56", "33", "39", "38", "40", "34", "51", "58", "57", "41", "54", "37", "53",
               "43", "42", "49", "55", "59", "44", "46", "36", "60", "52", "32", "50", "47", "35", "45"]
    ...
    def connect(n):
        connect_json = {"command": "subscribe",
                ...
                if ("identifier" in result) and ("message" in result):
                    ...
                    if "user" in b:
                        ...
                        if "messages" in b:
                            ...
                            db_update(id, name, username, message, room_id, "Message", strftime(
                                "%Y-%m-%d %H:%M:%S", gmtime()))
                        else:
                            if "user_disconnected" in b:
                                message = "None"
                                db_update(id, name, username, message, room_id, "Left", strftime(
                                    "%Y-%m-%d %H:%M:%S", gmtime()))
                            elif "Typing" in b:
                                message = "None"
                                db_update(id, name, username, message, room_id, "Typing", strftime(
                                    "%Y-%m-%d %H:%M:%S", gmtime()))
                            else:
                                message = "None"
                                db_update(id, name, username, message, room_id, "Joined", strftime(
                                    "%Y-%m-%d %H:%M:%S", gmtime()))
