...
def get_last_record_id(id, only_wfaf):
    start = time.perf_counter()
    if only_wfaf:
        query = f"""SELECT timestamp FROM latest_seen where unique_key = (SELECT MAX(unique_key) FROM latest_seen where id = {id} and room = 'WFAF')"""
    else:
        query = f"""SELECT id, name, username, room, timestamp FROM latest_seen where unique_key = (SELECT MAX(unique_key) FROM latest_seen where id = {id})"""
    result = query_runner(query)
    end = time.perf_counter()
    print(f"Query took {end - start} seconds")
    try:
