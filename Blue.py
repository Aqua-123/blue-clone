...
def replying_whos_idle():
    ...
    if len(idle_main_dict.keys()) == 0:
        ...
        response = whos_lurking_gen % str(whos_here_r)
    send_message(fix_message(response))
    ...
    for i in range(0, len(keys)):
        ...
        if bool(result) == True:
            if dictname == whos_here_res and re_m == whos_here:
                ...
                Thread(target=get_joke).start()
            elif dictname == response_dict and re_m == quote:
                Thread(target=get_quote).start()
            else:
...
def downvote(user_id, remem, id): requests.get(karma_url %
...
def stalker(id, time_now):
    ...
    if git_file in all_files:
        ...
        repo.update_file(logs.path, "stalker-log",
                         log, logs.sha, branch="main")
    else:
        repo.create_file(git_file, "committing files", "", branch="main")
    while stalking_log[id][1] == True:
        ...
        if r.status_code == 200:
            ...
            log = log + text
            repo.update_file(git_file, "committing files",
                             log, logs.sha, branch="main")
...
def respond_uptime():
    ...
    if sr[0] == "0":
        ...
        send_message(here_for_hours_and_mins % (sr[0], int(sr[1])+0))
...
def admin_func_init(i, id, isadmin, result,admin_id):
    ...
    if i == 0:
        ...
        send_stats
    elif i == 7 and isadmin == True:
        send_message(fix_message(str(mute_list)))
    elif i == 8 and isadmin == True:
    ...
    elif i == 11 and id not in mute_list:
        send_message(ily_r)
    elif i == 12 or i == 13:
        mute_func(result, i)
    elif i == 14:
        ...
        if not list1:
            ...
            send_message(stalking_following % fix_message(str(list1)))
    elif i == 19:
        aichatstate = True
        send_message(done)
    elif i == 20:
...
def coin_handling(result):
    ...
    if num.isdigit():
        ...
        if (coin_add < 101) and (coin_add > -1):
            ...
            if num == "1":
                send_message(adding_one_coin % (int(num) + 0, coin_new))
            else:
                send_message(adding_coins % (int(num) + 0, coin_new))
        elif coin_add > 100:
    ...
    if index == 4:
        ...
        for re_m in l:
            ...
            if result is not None:
                response = id_response % (name, list(
                    stats_list.keys())[l.index(re_m)])
                break
            else:
                n += 1
...
while running == True:
    try:
        ...
        if ("identifier" in a.keys()) and ("message" in a.keys()):
            ...
            Thread(target=greet, args=(
                "user_disconnected", "remove", False,)).start()
