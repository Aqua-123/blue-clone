...
def downvote(cookie, id_inp):
	r = requests.get(karma_url % id_inp, cookies=cookie, data=data_csrf, headers=head)
...
def thread(id_inp):
	banned.add(id)
	for i in cookiejar:
		cookie = {'_prototype_app_session': i,'cf_clearance':'X1BTFa1LSoIoira.B9EVDY0uTrFCb2.uiCxqi4WIehA-1659206546-0-150'}
...
def whos_here_appending(id_inp):
	try:
		resp = requests.get(profile_url % id_inp, cookies=cookies, timeout=0.2,data=data_csrf, headers=head)
		resp = json.loads(resp.text)
